...
import time
# from aperoconfig import TEST_MODE, DATA_DIR
...
def smallStack(commentaire='', deep=1):
...
#     frame = sys._getframe(2)
    ...
    while k<=deep[1] :
        try :
            filename = Path(sys._getframe(k).f_code.co_filename).namebase
            fonction = sys._getframe(k).f_code.co_name
#             lnum = sys._getframe(k).f_lineno
            toprint.append(u"%s.%s"%(filename,fonction))
            k += 1
        except :
            break
    toprint.reverse()
