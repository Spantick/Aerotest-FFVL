...
from math import pi as π, sqrt, sin, pi
...
from numpy import (linspace, radians, degrees, vstack, hstack, where, asarray,
                   ones_like, nan)
from matplotlib import pyplot as plt
from aperoconfig import DATA_DIR
import aperoconfig as AC
from model.basicobjects.pathbook import PathBook
from utils.utilitaires import className, ordered, maintenant, sapero
...
class Polaires(object) :
    ...
    de simulation (les .cmo) disponibles dans le répertoire
        ...
        Le point de reference pour les moments est RMPX,RMPY,RMPZ utilisé par CMARC.
        C'est le BA du parapente simulé.
    """
    def __init__(self, lookupdata=False, **kargs):
        session = kargs.pop('session') if 'session' in kargs else AC.SESSION
        self.pb = PathBook(session, sujet='aerodynamique')
        self.session = session
        self.files = set()
        # debug("Le fichier %s existe : %s"%(self.pb.polaires.name,self.pb.polaires.isfile()))
        if lookupdata or not self.pb.polairesfile.isfile():
            # debug("On re-scanne toutes les simulations")
            self.loadFromAllSimulations()
            self.dump() #on recrée le fichier polaires
            if kargs : self.load(kargs)
        else :
            # debug("On utilise le fichier polaires")
            try :
                D = self.loadFromFile()
                D.update(kargs)
                # debug(D)
                ...
                msg1 = "Impossible de lire le fichier polaires de la session %r"%(AC.SESSION)
                rdebug(msg1)
#                 self.loadFromAllSimulations()
#                 if kargs : self.load(kargs)
                raise
...
#         mexit()
    ...
    def setCollapseAndStall(self, 𝛂s=15, 𝛂c=None):
        ...
        if 𝛂c is None and 𝛂s is None :
            ...
            Cxs = asarray(self.Cxs[0:len(Czs)])
            Cxs[-1] *= 1.5
            # le dernier coeff de moment
            Cms = asarray(self.Cms[0:len(Czs)])
            Cms[-1] = 0
            # Cms[-1] = -1
            w = 5*ones_like(Czs)
            w[-2] = 0.001
...
#             coefs = F['Voile']
        ...
        self.areas, self.S0s, self.vinfs, self.b0s, self.l0s, self.refms) = T

        self.verifications()

    def verifications(self):
        '''Verification : surfaces, cordes, 1/2 envergures et points de reference pour les moments
        des différentes simulations doivent être identiques'''
        if len(set(self.refms))==1 :
            self.refm = self.refms[0]
            del self.refms
        else :
            msg = "Les points de reference pour les moments des differentes simulations ne sont pas identiques : \n  (alfa, refm)=%s"%list(zip(self.alfas, self.refms))
    ...
    def info(self):
        ...
        for attr in ('alfadegs', 'Czs', 'Cxs', 'Cms',
                     'yawdeg', 'area', 'vinf', 'S0', 'b0', 'l0', 'refm') :
    ...
    def toDump(self):
        return dict(alfadegs=self.alfadegs,
                    ...
                    S0=self.S0, b0=self.b0, l0=self.l0, refm=self.refm)
...
class Equilibriste(object):
    ...
    """
    VARIABLES = set(['d', 'b', 'S', 'l', 's', 'xGv', 'zGv', 'mp', 'mv', 'c', 'zs', 'Cx0', 'Cxp', 'Cxs',])
    VARIABLES = dict(
...
#         ma = 'Masse masse d\'air emprisonné.',
    ...
    def __init__(self, **kargs):
        ...
        """
        raise NotImplementedError("""29 Oct. 2019. Non fonctionnel, le point de référence pour les moments n'est pas utilisé.
...
#             dump = eval(f.read())
        ...
        calcul et sauvegarde des polaires %r"""%self.P.session)
            ...
            try : debug('%r existe : %s'%(self.P.polairesfile.name, not lookup))
            except AttributeError : pass
        if 'polaires' in kargs :
            pol = kargs['polaires']
        if self.P is not None :
            pol = Polaires(session=self.P.session, lookupdata=lookup)#.toDump()


        if lookup : pol.dump()
        dump.update(
            CxvCMARC = lambda alfa:pol.Cx(alfa%π),
            Cmv      = lambda alfa:pol.Cm(alfa%π),
            Cz       = lambda alfa:pol.Cz(alfa%π),
            b0=pol.b0, l0=pol.l0, vinf=pol.vinf, S0=pol.S0, session=self.session
            )
    ...
    @property
    def session(self):
        try : return self.parapente.session
...
#         alfa = alfa%(2*π)-π
#         alfa = alfa%π
        value = (2*self.m*g)/(ρ*self.Cz(alfa)*self.S0)
#         print (alfa)
        if value < 0 :
            value = -sqrt(-value)
...
#         debug(alfa_deg=degrees(alfa))
    ...
    def resultante(self, alfa):
        ...
        qS = self.qS(alfa)
        mg = self.m*g
        return -mg*γ+qS*self.CxT(alfa), -mg+qS*self.Cz(alfa)
    ...
    def equilibre(self,**kargs) :
        ...
        alfa, res = newton(objectif, radians(6.0), tol=self.EPS, full_output=True, disp=False)
        if res.converged :
            return alfa%π
        else :
            alfa = res.root
...
#         except RuntimeError as msg : return nan
        ...
        if res.converged :
            ...
            debug('alfaV : non CV')
            print("alfa_normalisé = %s,"%fmt(degrees(alfa%π)), 'resultante = (%s,%s),'%fmt((rn, r)), 'CmT =', cm)
...
if __name__ == '__main__':
    ...
    t = Tests(session)
    # show = True
    if 1 : t.testEquilibre(show=True)
